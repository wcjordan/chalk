...
def test_sample_fixture_ingestion():
    ...
    assert (
        first_chunk.chunk_id == "sample-chunk000"
    ), f"Expected 'sample-chunk000', got '{first_chunk.chunk_id}'"
    ...
    assert (
        first_chunk.start_time <= first_chunk.end_time
    ...
    assert first_chunk.metadata["num_events"] == len(
        first_chunk.events
    ), "num_events should match events list length"
    expected_duration = first_chunk.end_time - first_chunk.start_time
    assert (
        first_chunk.metadata["duration_ms"] == expected_duration
    ), "duration_ms should match time difference"

    # Verify we have reasonable chunk counts (sample has 2 FullSnapshots, so expect 2+ chunks)
    assert (
        len(chunks) >= 2
    ), f"Expected at least 2 chunks due to FullSnapshots, got {len(chunks)}"
...
def test_sample_fixture_event_types():
    ...
    assert (
        len(found_sources) >= 3
    ), f"Should find at least 3 interaction types, found: {found_sources}"
...
def test_sample_fixture_chunk_boundaries():
    ...
    for i, chunk in enumerate(chunks):
        ...
        assert (
            chunk.chunk_id == expected_id
        ), f"Expected chunk ID '{expected_id}', got '{chunk.chunk_id}'"
...
def test_sample_fixture_noise_filtering():
    ...
    # Don't filter any scrolls
    with patch("rrweb_ingest.config.MICRO_SCROLL_THRESHOLD", 0):
        chunks_unfiltered = ingest_session("sample", sample_path)
    ...
    assert (
        total_chunk_events <= total_unfiltered_events
