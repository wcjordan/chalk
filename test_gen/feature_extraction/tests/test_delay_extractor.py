...
import pytest
from feature_extraction.extractors import compute_inter_event_delays, compute_reaction_delays
...
def fixture_reaction_events_within_window():
    ...
    return [
        ...
        {"type": 3, "timestamp": 1800, "data": {"source": 0, "adds": []}},  # Mutation (800ms later)
...
def fixture_reaction_events_outside_window():
    ...
    return [
        {"type": 3, "timestamp": 1000, "data": {"source": 2, "id": 42}},  # Click
        {"type": 3, "timestamp": 12000, "data": {"source": 0, "adds": []}},  # Mutation (11000ms later)
    ]
...
def fixture_multiple_interactions():
    ...
    return [
        ...
        {"type": 3, "timestamp": 1100, "data": {"source": 0, "adds": []}},  # Mutation 1 (100ms later)
        {"type": 3, "timestamp": 2000, "data": {"source": 2, "id": 20}},  # Click 2
        {"type": 3, "timestamp": 2500, "data": {"source": 0, "texts": []}},  # Mutation 2 (500ms later)
...
def test_compute_reaction_delays_outside_window(reaction_events_outside_window):
    ...
    delays = compute_reaction_delays(
        reaction_events_outside_window, 
...
def test_compute_reaction_delays_custom_sources():
    ...
    delays = compute_reaction_delays(
        events, 
        interaction_source=5,  # Input events
...
def test_compute_reaction_delays_custom_max_window():
    ...
    events = [
        {"type": 3, "timestamp": 1000, "data": {"source": 2, "id": 42}},  # Click
        {"type": 3, "timestamp": 1600, "data": {"source": 0, "adds": []}},  # Mutation (600ms later)
    ]
...
def test_compute_reaction_delays_ignores_non_mutation_sources(mixed_source_events):
    ...
    events_with_fake_mutation = mixed_source_events + [
        {"type": 3, "timestamp": 1150, "data": {"source": 1, "adds": []}},  # Mouse move with 'adds'
    ]
...
def test_compute_reaction_delays_first_mutation_only():
    ...
    events = [
        {"type": 3, "timestamp": 1000, "data": {"source": 2, "id": 42}},  # Click
        {"type": 3, "timestamp": 1200, "data": {"source": 0, "adds": []}},  # First mutation
        {"type": 3, "timestamp": 1400, "data": {"source": 0, "texts": []}},  # Second mutation
    ]
...
def test_compute_reaction_delays_mutation_before_interaction():
    ...
    events = [
        {"type": 3, "timestamp": 1000, "data": {"source": 0, "adds": []}},  # Mutation first
...
def test_compute_reaction_delays_non_incremental_events():
    ...
    events = [
        {"type": 2, "timestamp": 1000, "data": {"source": 2}},  # FullSnapshot, not IncrementalSnapshot
        {"type": 3, "timestamp": 1100, "data": {"source": 2, "id": 42}},  # Click
        {"type": 4, "timestamp": 1200, "data": {"source": 0}},  # Meta event, not IncrementalSnapshot
        {"type": 3, "timestamp": 1300, "data": {"source": 0, "adds": []}},  # Mutation
    ]
