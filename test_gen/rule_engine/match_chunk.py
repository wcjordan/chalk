...
def process_chunk_file(
    ...
    try:
        ...
        if not isinstance(chunk_data, dict) or "chunk_id" not in chunk_data:
            print(f"Warning: Skipping {chunk_file.name} - missing chunk_id", file=sys.stderr)
            return 0, 0
        ...
        if detected_actions:
            ...
            if verbose:
                print(f"  Found {len(detected_actions)} actions from {unique_rules} rules")
        ...
        print(f"Warning: Skipping {chunk_file.name} - malformed JSON: {e}", file=sys.stderr)
        return 0, 0
...
def main():
    ...
    if input_path.is_file():
        ...
        if not chunk_files:
            print(f"Warning: No JSON files found in directory {input_path}", file=sys.stderr)
    else:
    ...
    print(f"{total_actions} actions detected from {unique_rules_count} rules across {files_processed} files")
