...
class TestCLIMatchChunk:
    """Test class for CLI match chunk functionality."""

    def create_sample_chunk(self, chunk_id: str, has_matching_interaction: bool = True) -> dict:
    ...
    def create_sample_rule(self) -> dict:
    ...
    def test_single_file_processing(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            rule_data = self.create_sample_rule()
            ...
            chunk_data = self.create_sample_chunk("test_chunk_123", has_matching_interaction=True)
            ...
            actions_count, rules_count = process_chunk_file(chunk_file, rules, output_dir, verbose=True)
    ...
    def test_directory_processing(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            chunk1_data = self.create_sample_chunk("chunk1", has_matching_interaction=True)
            with open(chunk1_file, "w") as f:
                json.dump(chunk1_data, f)
            
            chunk2_file = chunks_dir / "chunk2.json"
            chunk2_data = self.create_sample_chunk("chunk2", has_matching_interaction=False)
            ...
            printed_output = "".join(call.args[0] for call in mock_stdout.write.calls if call.args)
    ...
    def test_malformed_json_handling(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with open(bad_chunk_file, "w") as f:
            ...
            rules = load_rules(rules_dir)
            actions_count1, rules_count1 = process_chunk_file(bad_chunk_file, rules, output_dir)
            assert actions_count1 == 0
            assert rules_count1 == 0
            
            # Test processing file missing chunk_id
            actions_count2, rules_count2 = process_chunk_file(missing_id_file, rules, output_dir)
    ...
    def test_invalid_rules_directory(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            chunk_file = temp_path / "test_chunk.json"
            chunk_data = self.create_sample_chunk("test_chunk")
            with open(chunk_file, "w") as f:
                json.dump(chunk_data, f)
            ...
            test_args = [
                str(chunk_file),
                "--rules-dir", str(nonexistent_rules)
            ]
