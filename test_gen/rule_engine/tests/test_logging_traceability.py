...
def sample_event():
    ...
    return UserInteraction(
        action="input", timestamp=1234567890, target_id="input_1", value="test input"
    )
...
def mismatched_node():
    ...
    return UINode(
        id="button_1", tag="button", text="Click me", attributes={"type": "submit"}
    )
...
class TestRuleMatchLogging:
    ...
    def test_successful_match_logs_info(
        self, caplog, sample_rule, sample_event, sample_node
    ):
        ...
        with caplog.at_level(logging.INFO, logger="test_gen.rule_engine"):
            result = apply_rule_to_event_and_node(
                sample_rule, sample_event, sample_node, 0
            )
    ...
    def test_rule_evaluation_debug_logging(
        self, caplog, sample_rule, sample_event, sample_node
    ):
    ...
    def test_action_mismatch_debug_logging(self, caplog, sample_rule, sample_node):
        ...
        wrong_event = UserInteraction(
            action="click", timestamp=1234567890, target_id="input_1", value="test"
        )
    ...
    def test_tag_mismatch_debug_logging(
        self, caplog, sample_rule, sample_event, mismatched_node
    ):
    ...
    def test_missing_attributes_debug_logging(self, caplog, sample_event):
        ...
        missing_logs = [
            log for log in debug_logs if "Missing attributes" in log.message
        ]
...
class TestVariableExtractionLogging:
    ...
    def test_variable_extraction_failure_debug_logging(
        self, caplog, sample_event, sample_node
    ):
        ...
        failure_logs = [
            log for log in debug_logs if "Variable extraction failed" in log.message
        ]
        assert (
            len(failure_logs) >= 2
        )  # Should have failures for missing_attribute and invalid_path

        # Check specific failure messages
        failure_messages = [log.message for log in failure_logs]
        assert any(
            "missing_attribute" in msg and "node.attributes.nonexistent" in msg
            for msg in failure_messages
        )
        assert any(
            "invalid_path" in msg and "node.invalid.path" in msg
            for msg in failure_messages
        )
...
class TestChunkProcessingLogging:
    ...
    def test_chunk_processing_info_logging(self, caplog, sample_chunk, sample_rule):
        ...
        summary_logs = [
            log for log in info_logs if "Chunk processing complete" in log.message
        ]
    ...
    def test_chunk_processing_debug_logging(self, caplog, sample_chunk, sample_rule):
        ...
        interaction_logs = [
            ...
            if "Processing" in log.message and "interactions" in log.message
        ]
...
class TestVerbosityToggling:
    ...
    def test_default_warning_level_suppresses_info_debug(
        self, caplog, sample_rule, sample_event, sample_node
    ):
    ...
    def test_verbose_enables_info_logs(
        self, caplog, sample_rule, sample_event, sample_node
    ):
    ...
    def test_trace_match_enables_debug_logs(
        self, caplog, sample_rule, sample_event, sample_node
    ):
    ...
    def test_variable_truncation_in_logs(self, caplog, sample_event, sample_node):
        ...
        rule = Rule(
            ...
            match={"event": {"action": "input"}, "node": {"tag": "input"}},
            confidence=0.8,
