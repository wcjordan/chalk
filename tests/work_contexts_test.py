...
from helpers.label_helpers import (add_labels, dismiss_add_label_modal, get_todo_labels)
from helpers.todo_helpers import (add_todo, find_todos, list_todo_descriptions, wait_for_todo,
...
def test_work_contexts(page, todo_prefix):
    ...
    # Add label
    add_labels(page, todo_item, ['5 minutes'], dismiss_picker=False,
               open_picker=False)

    # Verify the todo is visible and label is added
    todos = find_todos(page, quick_todo_description)
    assert todos.count() == 1
    assert get_todo_labels(todos.first) == ['up next', '5 minutes']
    ...
    _verify_todo_in_work_context(page, todo_prefix, quick_todo_description, 'Quick Fixes')
    _verify_todo_in_work_context(page, todo_prefix, shopping_todo_description, 'Shopping')
    _verify_todo_in_work_context(page, todo_prefix, quick_todo_description, 'Up Next')
    _verify_todo_in_work_context(page, todo_prefix, planning_todo_description, 'Chalk Planning')
